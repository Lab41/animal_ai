...
from a3c_src.process_autoencoder import local_train, local_test
...
def get_args():
    ...
    parser.add_argument("--collect_observations", type=bool, default=False)
...
def train(opt):
    ...
    num_states = 3
    num_actions = 5
    global_model = ActorCriticAutoencoder(num_states, num_actions)
    ...
    for index in range(opt.num_processes):
        ...
        if index == 0:
            process = mp.Process(target=local_train, args=(index, opt, global_model, optimizer, True, observation_collector, testing))
        else:
            process = mp.Process(target=local_train, args=(index, opt, global_model, optimizer,False, observation_collector, testing))
        process.start()
